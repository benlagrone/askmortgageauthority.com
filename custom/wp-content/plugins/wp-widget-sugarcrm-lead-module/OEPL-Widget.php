...
class OEPL_Lead_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			array( 'description' => __( 'This Widget will submit data into your SugarCRM Lead module.', 'WP2SL' ) ) 
		);
	}
		...
		foreach ( $RS as $module ) {
			if ( $module['hidden'] === 'N' ) {
				...
				switch ( $module['data_type'] ) {
					case 'date':
						$jq_class = 'DatePicker nonHidden';
						$readonly = 'readonly';
						break;
					case 'datetimecombo':
						$jq_class = 'DateTimePicker nonHidden';
						$readonly = 'readonly';
						break;
					case 'file':
						$jq_class = 'files nonHidden';
						$readonly = '';
						break;
					default:
						$jq_class = 'nonHidden';
				...
				if ( $module['required'] === 'Y' ) {
					$jq_class      .= ' LeadFormRequired';
					$label_asterisk = ' <span class="required_cls">*</span>';                
				} else {
					$label_asterisk = '';
				}
				...
				if ( $module['wp_custom_label'] && $module['wp_custom_label'] !== '' ) {
					$label = $module['wp_custom_label'] . $label_asterisk;
				} else {
					$label = $module['wp_meta_label'] . $label_asterisk;
				}
				echo '<p>';
				echo '<label><strong>' . $label . ' :</strong></label><br>';
				echo WP2SL_getHTMLElement( $module['field_type'], $module['wp_meta_key'], $module['field_value'], $module['field_value'], '', $jq_class, $readonly );
				echo '</p>';
			} else {
	...
	public function form( $instance ) {
		...
		foreach ( $RS as $field ) {
			$field_val = isset( $instance[ $field['field_name'] ] ) ? $instance[ $field['field_name'] ] : '';

			if ( $field['data_type'] === 'date' ) {
				$jq_class = 'DatePicker widefat';
				$extra    = 'readonly';
			} elseif ( $field['data_type'] === 'datetimecombo' ) {
				$jq_class = 'DateTimePicker widefat';
				$extra    = 'readonly';
			} else {
				$extra    = '';
				$jq_class = 'widefat';
			...
			echo WP2SL_getHTMLElement( $field['field_type'], $this->get_field_name( $field['field_name'] ), $field['field_value'], $field_val, '', $jq_class, $extra );
			echo '</p>';
		} 
	}
