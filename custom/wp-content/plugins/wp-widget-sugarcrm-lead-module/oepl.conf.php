...
$objSugar->sugar_url  = OEPL_SUGARCRM_URL;
$objSugar->sugar_user = OEPL_SUGARCRM_ADMIN_USER; 
$objSugar->sugar_pass = OEPL_SUGARCRM_ADMIN_PASS; 
...
if ( $htaccess_protected === 'Y' ) {
	$objSugar->is_htaccess_protected = true; 
	$objSugar->htaccess_admin_user   = $htaccess_username;
	$objSugar->htaccess_admin_pass   = $htaccess_password;   
}
...
function WP2SL_frontend_script_load() {
	...
	wp_enqueue_script( 'admin_js', OEPL_PLUGIN_URL . 'js/admin.js', array( 'jquery' ), '1.0', true );
	wp_enqueue_script( 'Date_picker_js', OEPL_PLUGIN_URL . 'js/jquery.datetimepicker.js', array(), '1.0', true );
...
function WP2SL_PluginStyleJS() {
	...
	wp_enqueue_script( 'OpelJS', OEPL_PLUGIN_URL . 'js/admin.js', array( 'jquery' ), '1.0', true );
	wp_enqueue_script( 'Date_picker_js', OEPL_PLUGIN_URL . 'js/jquery.datetimepicker.js', array(), '1.0', true );
	...
	if ( isset( $_GET['page'] ) && $_GET['page'] === 'mapping_table' ) {
		...
		wp_enqueue_script( 'OEPL-field-list', OEPL_PLUGIN_URL . 'js/field-list.js', array(), '1.0', true );
	}
...
function WP2SL_CreateMenu() {
	add_menu_page( 'SugarSetting', 'Your SugarCRM', 'manage_options', 'SugarSetting', 'WP2SL_SugarSettings', OEPL_PLUGIN_URL . 'image/OEPL_plugin_logo.png', 98 );
...
function WP2SL_SugarSettings() {
	include_once OEPL_PLUGIN_DIR . 'SugarSettings.php';
}
